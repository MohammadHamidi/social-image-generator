...
class TestimonialLayout(PhotoLayoutEngine):
    ...
    def _create_background(self) -> Image.Image:
        ...
        if bg_mode == 'solid_color':
            color = tuple(self.background.get('color', [250, 250, 255]))
    ...
    def _create_simple_gradient(self) -> Image.Image:
        ...
        colors = gradient_config.get('colors', [[250, 250, 255], [255, 255, 255]])
        direction = gradient_config.get('direction', 'vertical')

        img = Image.new('RGB', (self.canvas_width, self.canvas_height))
        draw = ImageDraw.Draw(img)

        color1 = tuple(colors[0]) if len(colors) > 0 else (250, 250, 255)
